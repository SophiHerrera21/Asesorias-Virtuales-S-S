...
from .forms import UsuarioForm, SolicitarRecuperacionForm, CodigoRecuperacionForm, NuevaContrasenaForm, PerfilAprendizForm, PasswordChangeForm, AsesorRegistroForm
...
def crear_usuario(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            send_mail(
                subject='Bienvenido a S&S Asesorías Virtuales',
                message=f'''
...
def login_view(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            messages.error(request, f'El usuario con email {email} no existe.')
...
def dashboard_asesor(request):
    ...
    grupos = Grupo.objects.filter(asesor=asesor, activo=True)
    
    # Estadísticas
    total_aprendices = sum(grupo.aprendices.count() for grupo in grupos)
    pruebas_creadas = Prueba.objects.filter(asesor=asesor).count()
    pruebas_pendientes = Prueba.objects.filter(asesor=asesor, estado='pendiente').count()
    pqrs_pendientes = PQRS.objects.filter(asesor=asesor, estado='pendiente').count()
    ...
    # Grupos que están cerca de su capacidad máxima
    grupos_capacidad = grupos.filter(
        aprendices__count__gte=models.F('max_aprendices') - 2
    )
...
def dashboard_coordinador(request):
    ...
    # PQRS pendientes
    pqrs_pendientes = PQRS.objects.filter(estado='pendiente').order_by('prioridad', '-fecha_creacion')
    ...
    asesores_ocupados = Asesor.objects.filter(
        ...
        num_grupos=Count('grupos', filter=models.Q(grupos__activo=True))
...
def perfil_aprendiz(request):
    if request.method == 'POST':
        form = PerfilAprendizForm(request.POST, request.FILES, instance=request.user)
        if form.is_valid():
            form.save()
            messages.success(request, 'Perfil actualizado correctamente')
            return redirect('perfil_aprendiz')
    else:
        form = PerfilAprendizForm(instance=request.user)
...
def perfil_asesor(request):
    ...
    return render(request, 'usuarios/perfil_asesor.html', {'form': form, 'usuario': usuario})
...
def crear_grupo(request):
    ...
    if asesor.grupos.filter(activo=True).count() >= asesor.max_grupos:
...
def programar_reunion(request, grupo_id):
    ...
    grupo = get_object_or_404(Grupo, id=grupo_id, asesor=request.user.asesor, activo=True)
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if accion == 'activar':
                ...
                if nuevo_asesor.grupos.filter(activo=True).count() >= nuevo_asesor.max_grupos:
...
def reportes_coordinador(request):
    ...
    grupos_stats = {
        ...
        'por_materia': grupos.values('materia').annotate(
            total=Count('id'),
            activos=Count('id', filter=models.Q(activo=True)),
            inactivos=Count('id', filter=models.Q(activo=False))
        ),
        'capacidad': {
            ...
            'al_maximo': grupos.filter(
                aprendices__count=models.F('max_aprendices')
            ).count()
        }
    }
    ...
    asesores_stats = {
        ...
        'grupos_por_asesor': asesores.annotate(
            ...
            grupos_activos=Count('grupos', filter=models.Q(grupos__activo=True))
    ...
    aprendices_stats = {
        ...
        'activos': aprendices.filter(activo=True).count(),
        'inactivos': aprendices.filter(activo=False).count(),
        'por_programa': aprendices.values('programa').annotate(
            total=Count('id'),
            activos=Count('id', filter=models.Q(activo=True))
        ),
        'por_semestre': aprendices.values('semestre').annotate(
            total=Count('id'),
            activos=Count('id', filter=models.Q(activo=True))
        )
    }
...
def exportar_reporte(request):
    ...
    try:
        ...
        if tipo_reporte == 'grupos':
            headers = ['Grupo', 'Materia', 'Asesor', 'Estado', 'Capacidad', 'Aprendices', 'Fecha Creación']
            ...
            for row, grupo in enumerate(grupos, row_offset+1):
                ...
                ws.cell(row=row, column=2, value=grupo.materia)
            ...
            for row, asesor in enumerate(asesores, row_offset+1):
                ...
                ws.cell(row=row, column=4, value=asesor.grupos.filter(activo=True).count())
...
def exportar_pdf_usuarios_componentes_grupos(request):
    ...
    # Datos de grupos
    grupos = Grupo.objects.all()
    grupos_activos = grupos.filter(activo=True).count()
    grupos_inactivos = grupos.filter(activo=False).count()
    total_grupos = grupos.count()
    ...
    # Tabla de grupos
    data_grupos = [['Nombre', 'Materia', 'Asesor', 'Estado', 'Capacidad', 'Aprendices']]
    for g in grupos:
        data_grupos.append([
            g.nombre,
            getattr(g, 'materia', '-'),
...
def componentes_aprendiz(request):
    ...
    componentes = Componente.objects.filter(grupos__aprendices=aprendiz).distinct()
    
    context = {
        'componentes': componentes,
        'componentes_disponibles': Componente.objects.exclude(grupos__aprendices=aprendiz).distinct()
    }
...
def grupos_aprendiz(request):
    aprendiz = request.user.aprendiz
    grupos = Grupo.objects.filter(aprendices=aprendiz)
